...
public class NotificationScaleTests
{
    ...
    [InlineData(null)]
    [InlineData("enormous")]
    public void Parse_UnusableValue_FallsBackToTheDefault(string? text) => Assert.Equal(NotificationScale.Default, NotificationScale.Parse(text));
    ...
    [InlineData("384px")]
    [InlineData("22.5%")]
    public void ToString_RoundTripsThroughParse(string written) => Assert.Equal(written.Replace(" ", ""), NotificationScale.Parse(written).ToString());
    ...
    [Fact]
    public void ComputeWidth_ScreenPercent_MatchesTheOldAutomaticBehaviour() => Assert.Equal(384, NotificationWindow.ComputeWidth(NotificationScale.Default, 2560), 1);

    [Fact]
    public void ComputeWidth_Pixels_IsTheRequestedWidth() => Assert.Equal(500, NotificationWindow.ComputeWidth(NotificationScale.Pixels(500), 2560), 1);
    ...
    [Fact]
    public void ComputeWidth_DefaultOn1080p_IsNeverBelowTheDesignWidth() => Assert.Equal(NotificationScale.DesignWidth, NotificationWindow.ComputeWidth(NotificationScale.Default, 1920), 1);

    [Theory]
    [InlineData(1920)]
    [InlineData(1366)]
    [InlineData(960)]   // 1080p at 200% display scaling
    public void ComputeScale_NarrowDisplays_NeverDrawSmallerThanDesigned(double displayWidth) => Assert.True(NotificationWindow.ComputeScale(NotificationScale.Default, displayWidth) >= 1.0);
